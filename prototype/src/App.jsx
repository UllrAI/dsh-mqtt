...
const API_TOKEN = import.meta.env.VITE_DSH_MQTT_MANAGEMENT_TOKEN;
...
async function api(path, options = {}) {
  ...
  if (!response.ok) throw new Error(value.error ?? `管理 API 返回 ${response.status}`);
  return value;
}
...
export function App() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      loadData().catch((reason) => setError(reason.message));
    }, 15_000);
    return () => {
      active = false;
  ...
  return (
    <main className="shell" onClick={() => setActiveMenu(null)}>
      <header className="topbar">
        ...
        <button className="config-link" type="button" onClick={() => setModal('advanced')}>查看连接配置</button>
        <button className="icon-button top-close" type="button" aria-label="关闭设置"><X size={25} strokeWidth={1.7} /></button>
      </header>

      <div className="settings-layout">
        <nav className="settings-nav" aria-label="设置导航">
          <a href="#general"><Settings size={22} />通用设置</a>
          <a href="#models"><Database size={22} />模型</a>
          <a className="selected" href="#remote-worker" aria-current="page"><Radio size={22} />远程 Worker</a>
        </nav>
        ...
        <section className="content" id="remote-worker">
          ...
          {error && <div className="connection-banner error-banner"><CircleAlert size={18} /><span><strong>无法连接管理 API</strong>{error}</span><button type="button" onClick={() => loadData().catch((reason) => setError(reason.message))}>重试</button></div>}
          ...
          <section className="worker-identity" aria-labelledby="worker-name">
            ...
            <div className="worker-copy">
              <h2 id="worker-name">{status?.display_name ?? config?.display_name ?? 'Worker 状态不可用'}</h2>
              ...
              <p className="worker-subtitle">{status ? `Broker ${status.online ? '已连接' : '未连接'} · ${workspaceReady} 个工作区可用 · ${status.active_requests}/${status.request_capacity} 个任务` : '等待真实状态数据'}</p>
            </div>
            <div className="worker-actions">
              <button className="primary-button" type="button" onClick={openInvite} disabled={!status}><UserRoundPlus size={18} />添加控制端</button>
              <button className="secondary-button" type="button" onClick={checkStatus} disabled={isRefreshing}><RefreshCw className={isRefreshing ? 'spin' : ''} size={17} />检查状态</button>
            </div>
          </section>
              ...
              {authorizedControllers.length === 0 ? (
                ...
                <div className="list-row controller-row" key={controller.id}>
                  ...
                  <span className="menu-wrap" onClick={(event) => event.stopPropagation()}>
                    ...
                    {activeMenu === controller.id && <div className="context-menu" role="menu"><button type="button" role="menuitem" onClick={() => openController('permissions', controller)}>查看权限</button><button type="button" role="menuitem" className="danger-item" onClick={() => revokeController(controller).catch((reason) => setError(reason.message))}>撤销控制端</button></div>}
                  </span>
                </div>
              ))}
            </div>
          ...
          <button className="advanced-link" type="button" onClick={() => setModal('advanced')}><Settings size={19} />Broker 与高级设置 <ChevronRight size={18} /></button>
        </section>
      </div>
